...
const firebaseConfig = {
  ...
  authDomain: "electbot-adk.firebaseapp.com",
  projectId: "adk-genai-472412",
  storageBucket: "electbot-adk.appspot.com",
  messagingSenderId: "112365499000",
  appId: "1:112365499000:web:..."
};
  ...
  document.getElementById('login-btn')?.addEventListener('click', () => {
    auth.signInWithPopup(provider)
      .then((result) => {
        ...
        document.getElementById('login-btn').innerHTML = `<img src="${user.photoURL}" style="width:24px;border-radius:50%" alt="User Profile">`;
        console.log('User signed in:', user.displayName);
      }).catch((error) => console.error('Auth Error:', error));
  });
}

// ═══ Service Worker for Efficiency ═══
...
async function sendChat(message) {
  ...
  typing.className = 'message msg-bot typing-indicator';
...
async function startQuiz() {
  try {
    ...
    quizQuestions = [{ q: "Quiz unavailable. Please try again.", options: [], correct: -1, explanation: "" }];
  }
  currentQ = 0;
  score = 0;
...
window.initMap = function () {
  mapInitialized = true;
  initAutocomplete();
  // Re-trigger fallback check if we are already on the map section
...
function initMapFallback() {
  ...
  // Clear the basic map message if it was shown
  const mapEl = document.getElementById('google-map');
  if (mapEl && mapEl.innerHTML.includes('Map Preview')) {
    mapEl.innerHTML = '';
  }
  ...
  if (!map) {
    map = new google.maps.Map(document.getElementById('google-map'), {
      ...
      zoom: 5,
      styles: [
        { elementType: 'geometry', stylers: [{ color: '#03030b' }] },
